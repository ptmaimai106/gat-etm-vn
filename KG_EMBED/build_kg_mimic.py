    ...
    def augment_graph(self):
        ...
        for node in list(self.G.nodes()):
            ...
            if node_type in ['ICD9', 'ATC', 'CPT']:
                ...
                # Since G is undirected, use neighbors and level to find parent
                ancestors = []
                current = node
                current_level = self.G.nodes[current].get('level', float('inf'))

                while True:
                    # Get neighbors connected by hierarchical edges
                    neighbors = list(self.G.neighbors(current))
                    # Find parent: neighbor with lower level connected by hierarchical edge
                    parent = None
                    parent_level = current_level

                    for neighbor in neighbors:
                        # Check if edge is hierarchical
                        edge_data = self.G.get_edge_data(current, neighbor, {})
                        if edge_data.get('edge_type') == 'hierarchical':
                            neighbor_level = self.G.nodes[neighbor].get('level', float('inf'))
